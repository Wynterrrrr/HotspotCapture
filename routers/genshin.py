...
@router.get("/genshin")
async def get_genshin_hot(type: str = Query("1", description="榜单分类: 1(公告), 2(活动), 3(资讯)")):
    """获取原神最新消息"""
    cache_key = f"genshin_{type}"
    
    cached_data = cache.get(cache_key)
    if cached_data:
        return cached_data
    
    url = f"https://bbs-api-static.miyoushe.com/painter/wapi/getNewsList?client_type=4&gids=2&last_id=&page_size=20&type={type}"
    headers = {
    ...
    if not response:
        raise HTTPException(status_code=500, detail="获取原神最新消息失败")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"解析原神最新消息失败: {str(e)}")
